...
base.url <- 'https://www.omdbapi.com/ to find the base url'
...
# are available.
query.parameters <- list(key = movie, type ="json")
...
Director <- function(movie) {
  result <- paste(movie$Title, " was directed by ", movie$Director, sep = '')
  return (result)
}
